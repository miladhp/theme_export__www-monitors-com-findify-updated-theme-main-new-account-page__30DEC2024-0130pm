    ...
    function(response) {
      if ((response.status.toUpperCase() == 'OK') &&
        ...
        if (response.data.max_allowed_product_compare &&
          ...
          if (response.data.compare_collection_group_check_url) {
            monitorsCompareAppConfig['monitorsCompareCollectionGroupCheckURL'] = response.data.compare_collection_group_check_url;
          }
...
function initMonitorsProductCompareApp(monitorsCompareAppConfig)
{
  ...
  if (!montorsCaWrapper || null == montorsCaWrapper) {
    ...
    var collectionType = sessionStorage.getItem('collectionType');
    console.log({collectionValue, isCollection, collectionType});
    if (collectionType == 'sale' || isCollection == 'false') {
...
function initComparePopup(monitorsCompareAppConfig)
{
  ...
  document.querySelector('.ca_Rmvall').addEventListener('click', function(e) {
    ...
    resetCollectionGroupToLocalStorage(monitorsCompareAppConfig);
  ...
  document.querySelector('#monitors_ca_compare_products').addEventListener('click', function(e){
    ...
    let collectionGroupStorageName = 'collectionGroupId';
    let collectionGroupId = getSavedCollectionGroupId(collectionGroupStorageName);
    ...
    if (result.products &&
        ...
        for (let productkey in result.products) {
           ...
           var productPriceSection = '';
            ...
            let productPrice = productDetails.variant_price,
              productMinPrice = productDetails.product_min_variant_price,
              productMaxPrice = productDetails.product_max_variant_price,
              productComparePrice = productDetails.variant_compare_at_price;
            ...
            if (productComparePrice > productPrice) {
              productPriceSection = `<div class="findify-components--cards--product--price__price-wrapper findify-components--cards--product__price-wrapper money">`;
                if (productMinPrice !== productMaxPrice) {
                    productPriceSection += `<span class="findify-price_from sale-price">From</span>`;
                }
                productPriceSection += `<span class="findify-components--cards--product--price__price findify-components--cards--product--price__sale-price">
                ${productMinPrice}
                </span>
              ...
              productPriceSection = `<div class="findify-components--cards--product--price__price-wrapper findify-components--cards--product__price-wrapper">
                <span class="findify-components--cards--product--price__price">${productMinPrice}</span>
              </div>`;
            }
    ...
    if (result.products &&
        ...
        for (let productkey in result.products) {
            ...
            if (productComparePrice > productPrice) {
                ...
                if (productMinPrice !== productMaxPrice) {
                    productPriceSection += `<span class="findify-price_from sale-price custom-from-text">From</span>`;
                }
...
function productCompareCheckboxClickTrigger(monitorsCompareAppConfig, targetCompareElement)
{
  ...
  let collectionGroupStorageName = 'collectionGroupId';
  let savedCollectionGroupId = getSavedCollectionGroupId(collectionGroupStorageName);
  let monitorsCompareCollectionGroupCheckURL = '';

  if (monitorsCompareAppConfig['monitorsCompareCollectionGroupCheckURL']) {
    monitorsCompareCollectionGroupCheckURL = monitorsCompareAppConfig['monitorsCompareCollectionGroupCheckURL'];
  }

  console.log({savedCollectionList, savedProductTypeList, savedCollectionGroupId});
  ...
  if (targetCompareElement.checked) {
    console.log(`collectionId : ${collectionId}, savedCollectionList : ${savedCollectionList}`)
    if (savedCollectionList == "") {
      saveProductCollectionToLocalStorage(monitorsCompareAppConfig, collectionId);
      console.log(`productIndex savedCollectionList empty : ${productIndex}`);
      if (productIndex == -1) {
          ...
          function(response) {
            if ((response.status.toUpperCase() == 'OK') &&
              ...
              if (savedProductTypeList == "") {
                ...
                console.log(`Product compare is allowed only within same product type`);
      ...
      console.log(`productIndex savedCollectionList not empty 1 : ${productIndex}`);
      if (monitorsCompareCollectionGroupCheckURL != '') {/* collection group checking URL exists */
        let groupCheckingPayload = {};
        groupCheckingPayload['shop'] = monitorsProductCompareShop;
        groupCheckingPayload['group_id'] = '';
        groupCheckingPayload['collections'] = '';

        if (savedCollectionGroupId != '') { /* collection group exists in memory */
          groupCheckingPayload['group_id'] = savedCollectionGroupId;
          groupCheckingPayload['collections'] = collectionId;
        } else { /* need to check if both collections belong to same group */
          let collectionIdArray = [];
          collectionIdArray.push(savedCollectionList);
          collectionIdArray.push(collectionId);

          groupCheckingPayload['collections'] = collectionIdArray.join(',');
        }
        ...
        getDataUsingAjax(
          function(response) {
            ...
            if ((response.status.toUpperCase() == 'OK') &&
              ...
              if (response.data.group_id) {
                saveCollectionGroupIdToLocalStorage(monitorsCompareAppConfig, response.data.group_id);
              }
          ...
          monitorsCompareCollectionGroupCheckURL,
          'POST',
          {
            'payload': JSON.stringify(groupCheckingPayload)
          }
        );
      } else {
        // alert(`Product compare is allowed only within same collection and type`);
      ...
      console.log(`productIndex savedCollectionList not empty 2 : ${productIndex}`);
      if (productIndex == -1) {
        //Disable all checkboxes with class SPCMP_Add
        var checkboxes = document.querySelectorAll('.SPCMP_Add');
        for (var i = 0; i < checkboxes.length; i++) {
...
function renderAddedProductsInCompareList(monitorsCompareAppConfig, productsList)
{
  ...
  if (productsList.length) {
      ...
      for (let productInfo of productsList) {
          ...
          productMinPrice = productInfo.product_min_variant_price,
          productMaxPrice = productInfo.product_max_variant_price;
        
          productPrice = productPrice.substring(1);
          productComparePrice = productComparePrice.substring(1);
          productMinPrice = productMinPrice.substring(1);
          productMaxPrice = productMaxPrice.substring(1);
        
          productPrice = parseFloat(productPrice);
          productComparePrice = parseFloat(productComparePrice);
          productMinPrice = parseFloat(productMinPrice);
          productMaxPrice = parseFloat(productMaxPrice);
          ...
          if (productComparePrice > productPrice) {
              ...
              productPriceSection += `
                <span class="findify-components--cards--product--price__price findify-components--cards--product--price__sale-price">
                ${productInfo.product_min_variant_price}
                </span>
                <span class="findify-components--cards--product--price__compare">
...
function getSavedCollectionGroupId(collectionGroupStorageName)
{
  let savedCollectionGroupId = localStorage.getItem(collectionGroupStorageName);

  if (savedCollectionGroupId === '' || savedCollectionGroupId == null) {
    savedCollectionGroupId = "";
  }

  return savedCollectionGroupId;
}
...
function saveCollectionGroupIdToLocalStorage(monitorsCompareAppConfig, collectionGroupId)
{
  let collectionGroupStorageName = 'collectionGroupId';
  let savedCollectionGroupId = getSavedCollectionGroupId(collectionGroupStorageName);

  if (savedCollectionGroupId == "") {
    savedCollectionGroupId = collectionGroupId;
  }

  localStorage.setItem(collectionGroupStorageName, savedCollectionGroupId);
}
...
function resetCollectionGroupToLocalStorage(monitorsCompareAppConfig)
{
  let collectionGroupStorageName = 'collectionGroupId';
  localStorage.setItem(collectionGroupStorageName, "");
}
...
function heightsEqualizerAuto() {/*SET HEIGH IN RESPONSIVE*/
  ...
  if ($(window).width() <= 768) {
    $('.sp_product .slick-slide').each(function(){
        var $lis = $(this).find('ul.sp_product_con li');
        var maxHeights = [];

        $lis.each(function(index){
            var $currentLi = $(this);
            var currentHeight = $currentLi.outerHeight();
            var maxHeight = currentHeight;

            $('.slick-slide').not($(this).closest('.slick-slide')).each(function(){
                var $otherLi = $(this).find('ul.sp_product_con li').eq(index);
                if ($otherLi.length) {
                    var otherHeight = $otherLi.outerHeight();
        ...
        $lis.each(function(index){
            $(this).outerHeight(maxHeights[index]);
        });
    });
  }
