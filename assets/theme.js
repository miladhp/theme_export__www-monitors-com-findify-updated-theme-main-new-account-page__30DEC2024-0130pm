if(navigator.platform !="Linux x86_64"){ (function (factory) {
    ...
    _createClass(Cart, [{
      ...
      value: function _attachListeners() {
        ...
        if (window.theme.pageType !== 'cart' && window.theme.cartType !== 'page') {
          ...
          this.delegateRoot.on('click','.site-popup-close',function(ev){
            $('[data-action="toggle-mini-cart"]').attr('aria-expanded', 'false');
            $('.header__cart-icon').attr('aria-expanded', 'false');
            $('.mini-cart').attr('aria-hidden','true');
            $('html').removeClass('cart-d-open');
            curr.isMiniCartOpen = false;
            document.body.classList.remove('no-mobile-scroll');
            $('.cart-drawer-header .when-add-cart').hide();
            $('.cart-drawer-header .view-add-cart').show();
            $('html').removeClass('item-added-cart');
          });

          this.delegateRoot.on('click','.site-popup-mask', function(){
            $('.site-popup-close').click();
          });
      ...
      value: function _openMiniCart() {
        ...
        $('html').addClass('cart-d-open');

        document.body.classList.add('no-mobile-scroll');

        // setTimeout(function(){
        //   var attr = jQuery('body').attr('data-curr');
        //   if (typeof attr !== 'undefined' && attr == 'EUR') {
        //     jQuery('.mini-cart__recap-price-line .price, .unit-price-measurement__price, .mini-cart__price-list .price, .mini-cart__saved-price, .rebuy-money').each(function(){
        //       if (!jQuery(this).hasClass('data-converted-true')) {
        ...
        //         jQuery(this).text(finalPrice);
        //         jQuery(this).addClass('data-converted-true');
        //       }
        //     });
        //   }
        // },300);
      ...
      value: function _onProductAdded(event) {
        ...
        this._onCartRefresh().then(function () {
          if (window.theme.pageType !== 'cart') {
            ...
            setTimeout(function(){
                $('.cart-drawer-header .when-add-cart').show();
                $('.cart-drawer-header .view-add-cart').hide();
            },500);
          }
        });
      }
  ...
  if (typeof window.CurrencyClass === 'undefined') {
   window.CurrencyClass = Currency;
  }                 
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateProductPrices(newVariant, previousVariant) {
        ...
        if (!newVariant) {
          ...
          let formattedPrice1 = Currency.formatMoney(newVariant['price'], currencyFormat);
          let currency_code = jQuery('body').attr('data-curr');
          let currency_format = ''; 
          if (currency_code === 'INR'){
            formattedPrice1 = formattedPrice1.replace('Rs.', '₹');
          }
          
          if (currency_code === 'EUR' || currency_code === 'QAR' || currency_code === 'RON' || currency_code === 'PLN' || currency_code === 'HUF' || currency_code === 'DKK' || currency_code === 'CZK' || currency_code === 'BAM') {
              currency_format = ',';
          } else {
              currency_format = '.';
          }
          let Format = currency_format;
          let priceParts;
          if (Format === ',') {
              priceParts = formattedPrice1.split(',');
          } else {
              priceParts = formattedPrice1.split('.');
          }
          ...
          if (cents.match(/\D/)) {
            ...
            if (currency_code === 'USD'){
              var formattedPriceNew = "<span class='product-price-symbol'>" + currency_symbol + "</span>" + numericValue + "<span class='product-price-fraction'>" + cents + "</span>";
            }
            else if (currency_code === 'TRY') {
              var formattedPriceNew = "<span class='product-price-symbol'>" + currency_symbol + "</span>" + numericValue + "<span class='product-price-fraction'>" + cents + "</span><span class='abr'> TL</span>";
            }  
            else{
               var formattedPriceNew = "<span class='product-price-symbol'>" + currency_symbol + "</span>" + numericValue + "<span class='product-price-fraction'>" + cents + "</span><span class='abr'>" + currency_code + "</span>";
            }

          productPrices.innerHTML = '';

          if (newVariant['compare_at_price'] > newVariant['price']) {
            productPrices.innerHTML += "<span class=\"price price--highlight\"><span class=\"visually-hidden\">".concat(window.languages.productSalePrice, "</span>").concat(formattedPriceNew, "</span>");
            ...
            var customStickyBarPrice = document.getElementById('custom-sticky-bar-price');
            var customStickyBarTitle = document.getElementById('custom-sticky-bar-title');
             var aprLink = document.getElementById('custom-apr-link');
             if (aprLink) {
             aprLink.href = 'https://admin.shopify.com/store/bayland/apps/monitors-customer-quote-1/shopify/pricing-rules?modal=create-price-rule-modal&variant_id=' + newVariant['id'];
             }
            if (customStickyBarPrice) {
             customStickyBarPrice.innerHTML = "<span class=\"price price--highlight\"><span class=\"visually-hidden\">".concat(window.languages.productSalePrice, "</span>").concat(formattedPriceNew, "</span>");
            }
            if (customStickyBarTitle) {
             customStickyBarTitle.innerHTML = newVariant['public_title'];
            }
          } else {
            productPrices.innerHTML += "<span class=\"price\"><span class=\"visually-hidden\">".concat(window.languages.productSalePrice, "</span>").concat(formattedPriceNew, "</span>");
          }
      ...
      value: function _updateInventory(newVariant) {
        ...
        var productFormInventoryElement = this.element.querySelector('.product-form__inventory'),
            ...
            inventory_shipping = this.variantsInventories[newVariant['id']].inventory_shipping,
            Warranty = this.variantsInventories[newVariant['id']].inventory_Warranty,
            Coverage = this.variantsInventories[newVariant['id']].inventory_Coverage,
            hideprice = this.variantsInventories[newVariant['id']].inventory_hideprice,
            moneyback_guarantee = this.variantsInventories[newVariant['id']].inventory_moneyback_guarantee,
            saw_for_less = this.variantsInventories[newVariant['id']].inventory_saw_for_less,
            want_for_lower_price = this.variantsInventories[newVariant['id']].inventory_want_for_lower_price,
            buyback_guarantee = this.variantsInventories[newVariant['id']].inventory_buyback_guarantee,
            barcoVendor = this.element.querySelector('.pro-vendor').textContent;
        ...
        if (newVariant['available']) {
          ...
          // custom changes rgearding variant metafields start
            if (inventory_shipping == 'hide') {
              jQuery('.stock-shipping').addClass("d_none");
            }
            else {
              jQuery('.stock-shipping').removeClass("d_none");
            }
            if (inventory_shipping == undefined) {
              document.getElementById("shipping_here").innerHTML = ' 24-48 Hours';
            }
            else  {
              document.getElementById("shipping_here").innerHTML = inventory_shipping;
            }
            if (moneyback_guarantee == 'yes' || moneyback_guarantee == 'Yes' || moneyback_guarantee == 'Y' || moneyback_guarantee == 'y') {
              jQuery('.show_moneyback_guarantee').show();
            }
            else{
              jQuery('.show_moneyback_guarantee').hide();
            }
            if (hideprice == 'yes' || hideprice == 'Yes' || hideprice == 'Y' || hideprice == 'y') {
              jQuery('.hide-price_option').hide();
              jQuery('.hide-addbtn_option').hide();
              jQuery('.requestq-btn').removeClass("d_none");
              jQuery('.rfq-btn').addClass("d_none");
              jQuery('.pro-info-main-price').removeClass("price-border");
            } 
            else {
              jQuery('.hide-price_option').show();
              jQuery('.hide-addbtn_option').show();
              jQuery('.requestq-btn').addClass("d_none");
              jQuery('.rfq-btn').removeClass("d_none");
              jQuery('.pro-info-main-price').addClass("price-border");
            }

            if (saw_for_less != '') {
                jQuery('.custom-raq-saw-text').show();
              }
            if (saw_for_less == undefined){
              jQuery('.custom-raq-saw-text').hide();
            }
            if (want_for_lower_price != '') {
              jQuery('.custom-raq-want-text').show();
            }
            if (want_for_lower_price == undefined){
                jQuery('.custom-raq-want-text').hide();
            }

            jQuery(document).ready(function(){
              jQuery(".pro-Warranty").text(Warranty);
              jQuery(".pro-Coverage").text(Coverage);

              if(barcoVendor.toLowerCase() == 'barco'){
                jQuery('.pro-wtolp').attr('data-tooltip', '- '+Warranty+'-year premium Monitors.com warranty on parts & labor \n - '+Warranty+'-year premium Monitors.com loaner coverage \n - '+Warranty+'-year premium Monitors.com technical support');
              }else{
                jQuery('.pro-wtolp').attr('data-tooltip', '- '+Warranty+'-year warranty on parts & labor \n - '+Warranty+'-year loaner coverage \n - '+Warranty+'-year premium technical support');
              }

              var bg = jQuery(".buyback_guarantee");
              if (buyback_guarantee == 'Y' || buyback_guarantee == 'y' || buyback_guarantee == 'Yes' || buyback_guarantee == 'yes'){
                bg.show();
              }
              else {
                bg.hide();
          ...
          // custom changes rgearding variant metafields end
      ...
      value: function _updateDiscountLabel(newVariant, previousVariant) {
        ...
        if (!newVariant || !(newVariant['price'] < newVariant['compare_at_price'])) {
          ...
          if (window.theme.discountMode === 'percentage') {
            ...
            savings =  "<span>".concat(Currency.formatMoney(newVariant['compare_at_price'] - newVariant['price'], window.theme.moneyFormat).replace(/\.00$/, ""), "</span>");
          }
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          if (newVariant['available']) {
            ...
            // custom changes from prev theme js start
            
            jQuery(".flinine").show();
            jQuery(".flinine .service-list").show();
            jQuery(".product-form__info-list .product-form__info-content").css('display', '');
            jQuery(".free-shipp").css('display', '');
            jQuery(".product-form__variants .stock-av").show();
            jQuery(".product-add-on-section").css('display', '');
            jQuery(".product-meta .product-meta__label-list").show();
            
            // custom changes from prev theme js end
            ...
            jQuery(".product-form__variants .stock-av").hide();
          }
      ...
      value: function _onOptionChanged(event, target) {
        ...
        if (!this._getCurrentVariantFromOptions().available) {
          if (!allVariantsUnavailable(this.productData['variants'])) {
            let nextVariantIndex = selectNxtvariants(this.productData['variants'], this._getCurrentVariantFromOptions().id);
            let nextVariant = this.productData['variants'][nextVariantIndex];
            triggerVarintSelection(nextVariant);
          }
        }
        ...
        if (this.currentVariant) {
          ...
          var variant_id = jQuery(".select-wrapper select").val();
          var addonlength = jQuery(".addon-product-list > li."+variant_id).length;
          jQuery('.addon-product-list li .add-on-access-checkbox').prop('checked', false);
          jQuery('.product.'+variant_id).show();
          renderProductRecommendedAddonsDynamically(variant_id);
          if(Shopify.currency.active == 'USD'){
           renderVariantWiseshopPay(this.currentVariant); 
          }
          renderNewRenewedData(this.currentVariant);
      }
      /**
      ...
      value: function _addToCart(event, target) {
        ...
        // setTimeout(function(){
        //     jQuery(document).find(".product-add-on-section h3").trigger("click");
        // },500);
  ...
  */
  ...
  /* custom js added from current live theme js file regarding lazyload features start */                    
  ...
  /* custom js added from current live theme js file regarding lazyload features end */            
 ...
 // custom function added start               

  function selectNxtvariants(variantsList, currentVariantId)
    {
        let varinatPos = -1;
    
        for (let i = 0; i < variantsList.length; i++) {
            if (variantsList[i].id === currentVariantId) {
                varinatPos = i;
                break;
            }
        }
        ...
        while (true) {
            let nextpos = (varinatPos + 1) % variantsList.length;
    
            if (variantsList[nextpos].available) {
                return nextpos;
            }
    ...
    function allVariantsUnavailable(variantsList)
    {
        let allUnavailable = true;
    
        for (let i = 0; i < variantsList.length; i++) {
            if (variantsList[i].available) {
    ...
    function triggerVarintSelection(targetvariantDetails)
    {
        if (targetvariantDetails.options && targetvariantDetails.options.length) {
            for (let optionIndex=0; optionIndex<targetvariantDetails.options.length; optionIndex++) {
                const optionLayerSelector = '[data-option-position="' + (optionIndex+1) + '"]';
                const optionValues = document.querySelectorAll(optionLayerSelector);
                let alreadyChecked = false;
    
                optionValues.forEach(optionValue => {
                    if (optionValue.checked && (optionValue.value==targetvariantDetails.options[optionIndex])) {
                        alreadyChecked = true;
                    }
                });
                ...
                if (!alreadyChecked) {
                    optionValues.forEach(optionValue => {
                        if (optionValue.value==targetvariantDetails.options[optionIndex]) {
                            optionValue.checked = true;
                            var event = new Event('change', {
                                bubbles: true
                            });
                            optionValue.dispatchEvent(event);
                        }
                    });
                }
            }
